...
export type AiContentPart =
  ...
  | { readonly type: 'image'; readonly mediaType: string; readonly dataReference: string }
  | { readonly type: 'audio'; readonly mediaType: string; readonly dataReference: string };
...
export interface AiMessage {
  ...
  /** When role === 'assistant' and the turn proposed tools. Forbidden on role === 'tool'. */
...
export interface StructuredOutputRequirements {
  readonly name: string;
...
export interface StreamingRequirements {
...
export interface AiExecutionRequest {
  ...
  readonly structuredOutput?: StructuredOutputRequirements;
...
export interface AdapterFailure {
  readonly kind: 'authentication' | 'rate-limit' | 'context-limit' | 'invalid-request' | 'unavailable' | 'timeout' | 'unknown';
  readonly message: string;
  readonly retryable: boolean;
...
export interface AiProviderAdapter {
  readonly id: string;
  execute(request: AiExecutionRequest): Promise<NormalizedAiResult>;
...
export interface AiAdapterResolver {
  resolve(binding: CapabilityBinding): Promise<AiProviderAdapter>;
}
...
export interface AiDiagnostics {
  record(value: AiDiagnostic): void;
  get(id: string): AiDiagnostic | undefined;
  list(): readonly AiDiagnostic[];
}
export interface AiFact {
  readonly type: 'ai.completed' | 'ai.failed' | 'ai.stream.completed' | 'ai.stream.failed' | 'ai.stream.cancelled';
  readonly requestId: string;
  readonly executionId: string;
  readonly diagnosticId: string;
}
export interface AiEventPublisher {
  publish(fact: AiFact): Promise<void>;
}
export interface AiTelemetry {
  completed(adapterId: string, durationMs: number, usage: AiUsage): void;
  failed(adapterId: string, code: AiErrorCode, durationMs: number): void;
  streamed(adapterId: string, eventCount: number): void;
}
