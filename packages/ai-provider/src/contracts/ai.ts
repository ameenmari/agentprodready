...
export interface AiExecutionRequest {readonly requestId:string;readonly binding:CapabilityBinding;readonly context:ExecutionContext;readonly messages:readonly AiMessage[];readonly generation:GenerationRequirements;readonly structuredOutput?:StructuredOutputRequirements;readonly tools?:readonly AiToolDefinition[];readonly streaming?:StreamingRequirements;readonly metadata:Readonly<Record<string,string>>;readonly constraints:Readonly<Record<string,string|number|boolean>>;readonly signal?:AbortSignal;}
...
export interface AiFact {readonly type:'ai.completed'|'ai.failed'|'ai.stream.completed'|'ai.stream.failed'|'ai.stream.cancelled';readonly requestId:string;readonly executionId:string;readonly diagnosticId:string;}
