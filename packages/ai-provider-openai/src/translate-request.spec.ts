...
const config: OpenAiProviderConfig = Object.freeze({
  apiKey: 'test',
  model: 'gpt-4o-mini',
});
...
describe('translateRequest max output tokens', () => {
  it('omits max_completion_tokens when maximumOutputTokens is undefined', () => {
    const body = translateRequest(baseRequest(), config);
    expect(body.max_completion_tokens).toBeUndefined();
  });

  it('maps maximumOutputTokens to max_completion_tokens', () => {
